...
@implementation SpotterAnalyzer{
}
+ (SpotterAnalyzer *)sharedAnalyzer
{
    static SpotterAnalyzer *spotter = nil;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        spotter = [SpotterAnalyzer new];
...
{
     NSString* inference_result = RunInferenceOnImageResult(image.CGImage);
    NSLog(@"Result === %@",inference_result);
    dispatch_async(dispatch_get_main_queue(), ^{
    [[[UIAlertView alloc]initWithTitle:nil message:inference_result delegate:nil cancelButtonTitle:@"Ok" otherButtonTitles:nil, nil]show];
    });
...
bool PortableReadFileToProtol(const std::string& file_name,
...
NSString* FilePathForResourceNames(NSString* name, NSString* extension) {
